...
UX_SLAVE_CLASS_RNDIS    *rndis;
    ...
    switch (request)
    {
        ...
        case UX_DEVICE_CLASS_RNDIS_SEND_ENCAPSULATED_COMMAND    :
            ...
            /* The interrupt endpoint is discovered during control-interface
               activation.  Reject the request if it is unavailable, since
               the response notification requires that endpoint.  */
